...
def gold_gain_over_standard(
    ...
    ag_hits = set(agentic_ids) & set(gold_ids)
...
def extra_gold_coverage(
    ...
    extra = set(agentic_ids) - set(standard_ids_topk)
    if not extra:
        return 0.0
...
def extract_agentic_final_evidence(record: Dict[str, Any]) -> List[str]:
    ...
    second_round_ids = extract_agentic_second_round(record)
    if second_round_ids:
        return second_round_ids

    return extract_agentic_first_round(record)
...
def add_k_metrics(row: Dict[str, Any], retrieved_ids: List[str], gold_ids: List[str], k_values: List[int]) -> None:
    for k in k_values:
        hits = len(set(retrieved_ids[:k]) & set(gold_ids))
        row[f"hits_at_{k}"] = hits
        row[f"hit_at_{k}"] = 1 if hits > 0 else 0
        row[f"precision_at_{k}"] = precision_at_k(retrieved_ids, gold_ids, k)
        row[f"recall_at_{k}"] = recall_at_k(retrieved_ids, gold_ids, k)


def add_k_summary(summary: Dict[str, Any], rows: List[Dict[str, Any]], k_values: List[int]) -> None:
    if not rows:
        for k in k_values:
            summary[f"precision_at_{k}"] = 0.0
            summary[f"recall_at_{k}"] = 0.0
            summary[f"avg_hits_at_{k}"] = 0.0
            summary[f"hit_at_{k}"] = 0.0
        return

    n = len(rows)
    for k in k_values:
        summary[f"precision_at_{k}"] = sum(r[f"precision_at_{k}"] for r in rows) / n
        summary[f"recall_at_{k}"] = sum(r[f"recall_at_{k}"] for r in rows) / n
        summary[f"avg_hits_at_{k}"] = sum(r[f"hits_at_{k}"] for r in rows) / n
        summary[f"hit_at_{k}"] = sum(r[f"hit_at_{k}"] for r in rows) / n
...
def evaluate_run(
    ...
    standard_records_by_qid: Dict[str, Dict[str, Any]],  # kept for signature compatibility
    top_ks: Tuple[int, ...] = (1, 2, 4, 8),
    ...
    for rec in records:
        ...
        # Main retrieval list:
        # - agentic RAG: use second round as final metric source
        # - standard RAG / others: use normal retrieved list
        if "agentic_rag" in run_name:
            second_round_ids = extract_agentic_second_round(rec)
            if second_round_ids:
                retrieved_chunk_ids = second_round_ids
            else:
                final_evidence_ids = extract_agentic_final_evidence(rec)
                if final_evidence_ids:
                    retrieved_chunk_ids = final_evidence_ids
                else:
                    retrieved_chunk_ids = extract_agentic_first_round(rec)
        else:
        ...
        for k in top_ks:
            row[f"precision_at_{k}"] = precision_at_k(retrieved_chunk_ids, gold_chunk_ids, k=k)
            row[f"recall_at_{k}"] = recall_at_k(retrieved_chunk_ids, gold_chunk_ids, k=k)
    ...
    if per_question_rows:
        ...
        for k in top_ks:
            summary[f"precision_at_{k}"] = (
                sum(r[f"precision_at_{k}"] for r in per_question_rows) / len(per_question_rows)
            )
            summary[f"recall_at_{k}"] = (
                sum(r[f"recall_at_{k}"] for r in per_question_rows) / len(per_question_rows)
            )
    else:
        summary = {
            "system": run_name,
...
def main() -> None:
    ...
    for run_name, run_file in RUN_FILES.items():
        if "standard_rag" in run_name and run_file.exists():
    ...
    save_jsonl(RESULTS_DIR / "PReval_k1248_per_question.jsonl", all_rows)
    save_json(RESULTS_DIR / "PReval_k1248_summary.json", all_summaries)

    print("\nDone.")
    print(f"Saved per-question results to {RESULTS_DIR / 'PReval_k1248_per_question.jsonl'}")
    print(f"Saved summary results to {RESULTS_DIR / 'PReval_k1248_summary.json'}")
